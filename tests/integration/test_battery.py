...
async def test_per_battery_module_live_telemetry(hass, setup_storage_modules):
    ...
    assert (
        float(_state(hass, entry, "battery_module_1_discharge_energy_total")) == 1234.5
    )
...
async def test_per_battery_module_health_detail(hass, setup_storage_modules):
    ...
    assert (
        float(_state(hass, entry, "battery_module_1_discharge_capacity_total")) == 200.0
    )
    assert float(_state(hass, entry, "battery_module_1_cycle_count")) == 123.4
    assert (
        _state(hass, entry, "battery_module_1_internal_short_circuit") == "18"
    )  # 0x12
    assert (
        _state(hass, entry, "battery_module_1_internal_sox_correction") == "52"
    )  # 0x34
    assert (
        _state(hass, entry, "battery_module_1_derating_mode") == "Bus voltage too high"
    )
